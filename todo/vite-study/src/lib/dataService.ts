...
// 2. Add History
export const addHistoryItem = async (item: Omit<HistoryItem, 'id' | '_id' | 'createdAt' | 'updatedAt'>): Promise<HistoryItem> => {
...
export const checkDuplicateHistory = async (_userId: string, content: string): Promise<HistoryItem[]> => {
    try {
...
// 7. Upload Images
export const uploadImages = async (_userId: string, files: File[]): Promise<string[]> => {
...
export const fetchFeatures = async (_userId: string): Promise<FeatureItem[]> => {
